...
const Events = ({arr}: {arr:any}) => {
    return <section className="bg-white overflow-hidden rounded-2xl shadow-lg w-full min-h-[535px]">
        <h3 className="font-bold text-center bg-ocean text-white">Ближайшие События</h3>
        {
            arr.length !== 0 ?
            arr.map((el: { time: any; name: any; groups: any[]; all: { cabinet: any; }; }) => (
                <div className="h-12 gap-2 text-sm flex justify-between items-center gap-2 border-b border-b-black px-2 last:border-0 w-auto">
